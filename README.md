...
```
agent/
  agent_client.py        # reads agent.yaml, creates/updates the Microsoft Foundry agent
  config/
...
cp .env.example .env
# Edit .env — set AZURE_AI_PROJECT to your Microsoft Foundry project endpoint

python -m agent.agent_client   # create/update agent in Microsoft Foundry
...
1. `agent_client.py` reads `agent.yaml` and creates a versioned agent in Microsoft Foundry
2. `run_eval_gate.py` submits `test_data.jsonl` as inline content to the Microsoft Foundry Evals API
3. A cloud eval targets the agent by name (`type: azure_ai_agent`; Microsoft Foundry calls it server-side for every row)
4. Microsoft Foundry runs all evaluators server-side using the judge model
5. `run.report_url` links directly to the run in the Microsoft Foundry Evaluations UI
...
| `AZURE_AI_PROJECT` | Production Microsoft Foundry project endpoint |
| `AZURE_AI_PROJECT_TEST` | Test Microsoft Foundry project endpoint (can be same as prod for single-resource setups) |

> **Note:** The project endpoints require that you've created projects inside your Microsoft Foundry resources (not just the resources themselves). See [SETUP.md](SETUP.md) for the full provisioning steps including project creation.
